...
class Home{
   ...
   public function shield( $string ){
     ...
     * +------------------------------------------------------------------------
     */
	public function getadvpic($db,$ctag) {
		if ($db!='' && $ctag!='') {
			$data = Db::name($db)->field('Id,pic,topic,linkurl')->where(['enabled'=>1,'ctag'=>$ctag])->order('ord ASC')->select();
		  return $data;
		} else {
		  return ;
		}
   ...
   /* -
     ...
     * +------------------------------------------------------------------------
     */
	public function getyears($db,$sty) {
		if ($db!='' && $sty!='') {
			$data = Db::name($db)->field('Id,topic')->where(['enabled'=>1,'sty'=>$sty])->select();
		  return $data;
		} else {
		  return ;
		}
    ...
    //检测视频免费
	public function ckinftype($db,$id) {
		if ($db!='' && $id!='') {
			$data = Db::name($db)->field('Id,inftype,topic')->where(['enabled'=>1,'Id'=>$id])->find();
		  return $data['inftype']==11 ? true : false;
		} else {
		  return false;
		}
	  }
...
function getpage($conf=''){
  $inftype    = isset($conf['inftype'])    ? intval($conf['inftype']) : 0;
  $order  = isset($conf['order'])  ? $conf['order']       : 'istop DESC,ord ASC,date DESC';
  ...
  if ($Id && $tables!='') {
   ...
   if($inftype) $where .= ' AND inftype='.$inftype;
   if(!$dlist = cache('dlist'.$tables.$inftype)) {
     $dlist = DB::name($tables)->field('Id')->where($where)->order($order)->select();
     cache('dlist'.$tables.$inftype,$dlist,3600);
   }
   if($dlist) {
 ...
 return $return;
   ...
   public function recordonline(){
	...
	//报名中心
	public function upapply($data){
		if($data){
			$result = Db::name('apply')->insert($data);
